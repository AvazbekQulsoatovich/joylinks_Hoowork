...
def setup_data():
    ...
    if not User.objects.filter(username='admin').exists():
        ...
        admin.role = 'ADMIN'
        # give the admin a huge coin balance so they can fund everyone
        admin.coin_balance = 1_000_000_000_000  # one trillion
        admin.save()
        print("Superuser 'admin' created (pass: admin123, coins: 1000000000000)")
    else:
        admin = User.objects.get(username='admin')
        if admin.coin_balance < 1_000_000_000_000:
            admin.coin_balance = 1_000_000_000_000
            admin.save(update_fields=['coin_balance'])
            print("Superuser 'admin' coin balance set to 1000000000000")
